...
function refreshScrollAnimations() {
  ...
  document.querySelectorAll('.reveal-up, .reveal-card, .reveal-statement, .reveal-row').forEach((el) => {
    if (el.dataset.revealed) gsap.set(el, { opacity: 1, x: 0, y: 0, clearProps: 'willChange' });
  });
  initScrollReveals();
  initNavLinks();
  initNavScroll();
...
function animateHero() {
  ...
  gsap.delayedCall(1.5, () => hl?.classList.add('bar-visible'));
}
...
function initScrollReveals() {
  ...
  document.querySelectorAll('.track-card').forEach((card, i) => {
    ...
    gsap.fromTo(card, { opacity: 0, y: 24 }, {
      ...
      scrollTrigger: {
        trigger: card, start: 'top 90%', toggleActions: 'play none none none',
  ...
  document.querySelectorAll('.reveal-statement').forEach((el) => {
    if (el.dataset.revealed) return;
    const delay = parseFloat(el.dataset.delay || 0);
    gsap.fromTo(el, { opacity: 0, x: slideFrom(80) }, {
      opacity: 1, x: 0, duration: 0.9, delay, ease: 'expo.out',
      scrollTrigger: {
        trigger: el, start: 'top 85%', toggleActions: 'play none none none',
        onEnter() {
          el.dataset.revealed = '1';
          document.querySelector('.statement-line.highlight-bar-full')?.classList.add('bar-visible');
        },
      },
    });
  });
  ...
  document.querySelectorAll('.reveal-row').forEach((el) => {
    if (el.dataset.revealed) return;
    const delay = parseFloat(el.dataset.delay || 0);
    gsap.fromTo(el, { opacity: 0, x: slideFrom(20) }, {
...
function initCounters() {
  document.querySelectorAll('.stat-number').forEach((el) => {
    ...
    gsap.fromTo({ val: 0 }, { val: num }, {
      val: num, duration: 1.4, ease: 'power2.out', snap: { val: 1 },
...
function initCursor() {
  ...
  dot.style.cssText = 'position:fixed;top:0;left:0;pointer-events:none;width:8px;height:8px;background:#fff824;border-radius:0;z-index:9999;mix-blend-mode:multiply;transition:width .2s,height .2s;transform:translate(-50%,-50%)';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (savedLang !== 'en') setLanguage(savedLang, false);
